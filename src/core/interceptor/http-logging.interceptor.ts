...
export class HttpLoggingInterceptor implements NestMiddleware {
	...
	use(request: Request, response: Response, next: NextFunction): void {
		...
		response.on('finish', () => {
			...
			this.httpRequestDuration.labels(originalUrl, method, statusCode.toString()).observe(responseTime);

			this.responseSizeHistogram.labels(originalUrl, method, statusCode.toString()).observe(responseContentLength);

			this.requestSizeHistogram.labels(originalUrl, method, statusCode.toString()).observe(requestContentLength);

			this.httpCounter.labels(originalUrl, method, statusCode.toString()).inc(1);

			this.logger.log(`[${method}] | ${originalUrl} | ${statusCode}  | size ${responseContentLength} |  delay ${responseTime}ms -  ${ip}`);
		});
